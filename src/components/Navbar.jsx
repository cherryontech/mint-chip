// src/components/Navbar.jsx (Ajustado para Menú Móvil con Color Sólido)
import React, { useState } from 'react'; // IMPORTANTE: Importar useState
import Button from './Button'; 

// SVG de 4 barras (tu icono de 'Menú')
const FourBarsIcon = (props) => (
  ...
  >
    <path 
      ...
      fill="currentColor" // Usar currentColor para heredar el color de Tailwind
    />
  </svg>
);

// SVG de la 'X' (ícono de 'Cerrar')
    ...
    return (
        <div className="w-full bg-nyanza relative z-20"> 
            ...
            <div className="flex justify-between items-center px-4 py-4 md:px-12 md:py-6 h-20">
                ...
                {/* Logo */}
                <div className="text-stone-900 text-2xl md:text-2xl font-normal font-playfair">
                    Healie
                </div>

                {/* Navegación de ESCRITORIO */}
                <nav className="hidden md:flex items-center gap-4">
                    <Button isNavbar={true} onClick={() => console.log('Go to sign up')} aria-label="Sign Up">
                        Sign Up
                    </Button>
                    <Button isNavbar={true} onClick={() => console.log('Go to sign in')} aria-label="Log In">
                        Log In
                    </Button>
                </nav>
            ...
            <div className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'} 
                ...
                <div className="flex flex-col gap-4 items-center">
                    <Button isNavbar={true} onClick={() => console.log('Go to sign up')} aria-label="Sign Up" className="w-full">
                        Sign Up
                    </Button>
                    <Button isNavbar={true} onClick={() => console.log('Go to sign in')} aria-label="Log In" className="w-full">
                        Log In
                    </Button>
                </div>
            </div>
